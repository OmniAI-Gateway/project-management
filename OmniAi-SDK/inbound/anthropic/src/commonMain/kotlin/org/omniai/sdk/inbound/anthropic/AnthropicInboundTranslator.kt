        ...
        return AnthropicMessageResponse(
            ...
            content =
                firstChoice
                    ?.message
                    ?.content
                    ?.mapNotNull(::toAnthropicContentPart)
                    .orEmpty(),
...
private fun commonRequestTypedMapInputs(clientRequest: AnthropicMessagesRequest): TypedMap =
    TypedMap().apply {
    ...
    when (this) {
        ...
        is ToolCallStartedEvent -> {
            AnthropicStreamEvent.ContentBlockStart(
                index = toolCallIndex,
                contentBlock =
                    AnthropicOutputContent.ToolUse(
                        id = toolCallId,
                        name = functionName,
...
private fun AnthropicContent?.toPlainSystemText(): String? =
    when (this) {
        null -> {
            null
        }
...
private fun AnthropicInputContentBlock.toDomainPart(): RequestContentPart? =
    when (this) {
        ...
        is AnthropicInputContentBlock.ToolUse -> {
            ToolCallPart(
                toolCallId = id,
                functionName = name,
        ...
        is AnthropicInputContentBlock.ToolResult -> {
            ToolResultPart(
        ...
        is AnthropicInputContentBlock.Thinking -> {
            null
        }
    }
...
private fun toAnthropicContentPart(part: ResponseContentPart): AnthropicOutputContent? =
    when (part) {
        is TextPart -> {
            AnthropicOutputContent.Text(text = part.text)
        }
        ...
        is JsonPart -> {
            AnthropicOutputContent.Text(text = part.json.toJsonString())
        }
...
private fun CommonUsage.toAnthropicUsage(): AnthropicUsage =
    AnthropicUsage(
