...
class AnthropicInboundAdapterTest {
    @Test
    fun `generate translates anthropic request and maps service response`() =
        runTest {
            var receivedRequest: CommonRequest? = null

            val service =
                object : DispatcherPort {
    ...
    @Test
    fun `generateStream delegates to service and maps domain events`() =
        runTest {
            val service =
                object : DispatcherPort {
                    override suspend fun generate(
                        request: CommonRequest,
                        attributes: TypedMap,
                    ): Either<DomainError, CommonResponse> = throw UnsupportedOperationException("not used in this test")
    ...
    private fun anthropicRequest(): AnthropicMessagesRequest =
