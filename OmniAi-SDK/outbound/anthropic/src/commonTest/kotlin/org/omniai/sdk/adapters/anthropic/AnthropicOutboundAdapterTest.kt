...
class AnthropicOutboundAdapterTest {
    ...
    @Test
    fun `generate sends expected request and maps success`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
                    executeResult =
    ...
    @Test
    fun `generate maps complex request configuration accurately`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
                    executeResult =
    ...
    @Test
    fun `generate maps HttpCallResult ApiError to DomainError`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    @Test
    fun `generate maps HttpCallResult NetworkError to DomainError`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    @Test
    fun `generateStream maps complete successful stream lifecycle`() =
        runTest {
    ...
    @Test
    fun `generateStream maps Anthropic specific inner error event to domain errored event`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    @Test
    fun `generateStream maps transport api error to domain error event`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    @Test
    fun `generateStream maps transport api error without message to default message`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    @Test
    fun `generateStream maps transport NetworkError to domain errored event`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    private fun createAdapter(fakeClient: FakeHttpTransportClient) =
    ...
    private fun commonRequest(): CommonRequest =
        CommonRequest(
            provider = Provider.ANTHROPIC,
            model = "claude-3-5-sonnet",
            messages =
...
private class FakeHttpTransportClient : HttpTransportClient {
    ...
    var listenResult: Flow<HttpCallResult<*>> =
        flowOf(
            HttpCallResult.Success(
