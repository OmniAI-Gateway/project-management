...
class OpenAiOutboundAdapterTest {
    ...
    @Test
    fun `generate maps api error to invalid request`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
    ...
    @Test
    fun `generateStream enables stream and maps chunk plus done event`() =
        runTest {
            val fakeClient =
                FakeHttpTransportClient().apply {
                    listenResult =
    ...
    @Test
    fun `generate merges response metadata into provider options`() =
        runTest {
...
private class FakeHttpTransportClient : HttpTransportClient {
    ...
    var listenResult: Flow<HttpCallResult<*>> =
        flowOf(HttpCallResult.UnknownError(IllegalStateException("listenResult not set")))
