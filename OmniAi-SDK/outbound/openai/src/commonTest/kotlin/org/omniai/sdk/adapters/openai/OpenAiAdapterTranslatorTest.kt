...
class OpenAiAdapterTranslatorTest {
    ...
    fun `maps common request to openai request`() {
        val request = CommonRequest(
            ...
            config = org.omniai.sdk.domain.common.CommonGenerationConfig(maxTokens = 120)
        )
    ...
    fun `maps tool call arguments JSON string to domain object`() {
        ...
        val domain = translator.toDomain(response)
        val part = domain.choices.first().message.content[0] as org.omniai.sdk.domain.common.content.ToolCallPart
        assertEquals("Lisboa", (part.argumentsJson["city"] as org.omniai.sdk.domain.common.json.JsonValue.JsonString).value)
    }
