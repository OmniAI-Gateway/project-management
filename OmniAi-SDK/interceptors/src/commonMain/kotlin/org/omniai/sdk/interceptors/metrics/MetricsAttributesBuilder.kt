...
class MetricsAttributesBuilder {
    ...
    fun include(key: AttributeKey<*>, alias: String = key.name) {
        extractors += { context, _ ->
            context.attributes.readAny(key)
                ?.toMetricTag()
    ...
    fun attribute(name: String, extractor: (GatewayContext, PipelineResult?) -> String?) {
...
private fun Any.toMetricTag(): String? = when (this) {
    is String -> this
    is Number -> toString()
    is Boolean -> toString()
    else -> null
