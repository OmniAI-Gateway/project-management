...
import org.omniai.sdk.interceptors.auth.authenticators.PassThroughTokenAuthenticator
...
import org.omniai.sdk.interceptors.auth.client.HttpAuthSecurityClient
import org.omniai.sdk.interceptors.auth.client.HttpIntrospectionClient
import org.omniai.sdk.interceptors.auth.client.JwksClient
...
import org.omniai.sdk.interceptors.auth.engines.PlatformJwtVerificationEngine
...
import org.omniai.sdk.interceptors.authentication.authenticators.DefaultTokenAuthenticator
...
internal suspend fun loadTokenAuthenticator(
    ...
    when (config) {
        ...
        is AuthSetupConfig.Discovery -> {
            ...
            val metadata =
                when (val result = discovery.fetchMetadata(config.discoveryUrl)) {
                    ...
                    is HttpCallResult.NetworkError -> throw RuntimeException(
                        "Network Error no Discovery de OIDC: ${result.exception.message}",
                        result.exception,
                    )
            ...
            val infraConfig =
                HttpAuthSecurityClientConfig(
                    authClientId = config.authClientId,
                    authClientSecret = config.authClientSecret,
                )

            val resolvedCache =
                introspectionCache ?: InMemoryIntrospectionCache(
            ...
            val introspectionClient =
                metadata.introspectionEndpoint?.let { endpoint ->
            ...
            val authInfra =
                HttpAuthSecurityClient(
                    jwksClient = jwksClient,
                    introspectionClient = introspectionClient,
                )

            val authenticator =
                DefaultTokenAuthenticator(
                    infra = authInfra,
