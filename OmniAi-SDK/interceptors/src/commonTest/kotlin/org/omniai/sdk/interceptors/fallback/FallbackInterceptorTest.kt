...
class FallbackInterceptorTest {
    ...
    @Test
    fun `primary succeeds result returned immediately, no fallback used`() = runTest {
    ...
    @Test
    fun `primary error triggers fallback to alternative outbound`() = runTest {
        val primary = FakeOutbound(Provider.OPENAI, "gpt-4o",
        ...
        var callCount = 0
        val chain = object : org.omniai.sdk.application.pipeline.InterceptorChain {
    ...
    @Test
    fun `fallback context has provider and model rewritten to alternative outbound`() = runTest {
        val primary = FakeOutbound(Provider.OPENAI, "gpt-4o")
        val alternative = FakeOutbound(Provider.ANTHROPIC, "claude-3")
        val cb = interceptor(listOf(primary, alternative))
    ...
    @Test
    fun `all outbounds fail last error is returned`() = runTest {
        val primary = FakeOutbound(Provider.OPENAI, "gpt-4o")
        val alternative = FakeOutbound(Provider.ANTHROPIC, "claude-3")
        val cb = interceptor(listOf(primary, alternative))
        ...
        val chain = object : org.omniai.sdk.application.pipeline.InterceptorChain {
            override suspend fun proceed(context: org.omniai.sdk.application.pipeline.GatewayContext): PipelineResult {
                callCount++
                return if (callCount == 1) PipelineResult.Error(fakeError("primary down"))
                else PipelineResult.Error(lastError)
            }
        }
    ...
    @Test
    fun `pre-denied primary outbound is skipped and alternative is tried directly`() = runTest {
        val primary = FakeOutbound(Provider.OPENAI, "gpt-4o")
        val alternative = FakeOutbound(Provider.ANTHROPIC, "claude-3")
        val cb = interceptor(listOf(primary, alternative))
    ...
    @Test
    fun `no outbounds at all returns NoResult`() = runTest {
