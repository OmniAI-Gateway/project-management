...
class RoutingInterceptorTest {
    ...
    @Test
    fun `empty outbounds returns Error with descriptive message`() =
        runTest {
            ...
            assertTrue(
                result.error.message.contains("outbound", ignoreCase = true),
    ...
    @Test
    fun `single outbound is always selected`() =
        runTest {
    ...
    @Test
    fun `single outbound context request is rewritten with outbound provider and model`() =
        runTest {
    ...
    @Test
    fun `multiple outbounds exactly one is selected per call`() =
        runTest {
            val outbounds =
                listOf(
            ...
            repeat(20) {
                ...
                interceptor.handle(fakeContext(), capturing)
                val selected =
                    capturing.lastContext
                        ?.request
                        ?.provider
                        ?.value
                assertNotNull(selected)
                assertTrue(
                    selected in validProviders,
    ...
    @Test
    fun `routing preserves existing context attributes`() =
        runTest {
            val outbound = FakeOutbound(Provider.OPENAI, "gpt-4o")
            val interceptor = RoutingInterceptor(listOf(outbound))
            val capturing = CapturingChain()
    ...
    @Test
    fun `routing preserves request mode`() =
        runTest {
            val outbound = FakeOutbound(Provider.OPENAI, "gpt-4o")
            val interceptor = RoutingInterceptor(listOf(outbound))
            val capturing = CapturingChain()
    ...
    @Test
    fun `result from chain is returned as-is`() =
        runTest {
            val outbound = FakeOutbound(Provider.OPENAI, "gpt-4o")
            val interceptor = RoutingInterceptor(listOf(outbound))
