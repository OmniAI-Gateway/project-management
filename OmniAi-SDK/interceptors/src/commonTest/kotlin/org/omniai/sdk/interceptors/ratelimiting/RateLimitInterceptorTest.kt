...
class RateLimitInterceptorTest {
    ...
    private fun definition(limit: Int = 100) = RateLimitDefinition(limit = limit, type = RateLimitType.REQUESTS, window = window())
    ...
    private fun alwaysThrottle(retryAfter: Duration = 5.seconds) =
        object : RateLimitStore {
            override suspend fun consume(
                target: RateLimitTarget,
                weight: Int,
    ...
    @Test
    fun `allowed request passes through to chain`() =
        runTest {
    ...
    @Test
    fun `no policies request always passes through`() =
        runTest {
            val chain = StaticChain(PipelineResult.Unary(fakeResponse()))
    ...
    @Test
    fun `policy with null extract is skipped and request passes through`() =
        runTest {
            val chain = StaticChain(PipelineResult.Unary(fakeResponse()))
            val interceptor =
                RateLimitInterceptor(
    ...
    @Test
    fun `throttled request returns Error with TooManyRequestsError`() =
        runTest {
            val chain = StaticChain(PipelineResult.Unary(fakeResponse()))
            val interceptor =
                RateLimitInterceptor(
    ...
    @Test
    fun `throttled request carries the retryAfter duration from the store`() =
        runTest {
            val interceptor =
                RateLimitInterceptor(
    ...
    @Test
    fun `when multiple policies throttle, the largest retryAfter wins`() =
        runTest {
    ...
    @Test
    fun `one allowed policy and one throttled policy results in throttle`() =
        runTest {
            val interceptor =
                RateLimitInterceptor(
                    store =
                        object : RateLimitStore {
                            ...
                            override suspend fun consume(
                                ...
                                if (callCount++ == 0) {
                                    ...
                                    RateLimitResult.Throttled(3.seconds)
                                }
                        },
                    policies =
                        listOf(
    ...
    @Test
    fun `skipped policy mixed with throttled policy still throttles`() =
        runTest {
            val interceptor =
                RateLimitInterceptor(
