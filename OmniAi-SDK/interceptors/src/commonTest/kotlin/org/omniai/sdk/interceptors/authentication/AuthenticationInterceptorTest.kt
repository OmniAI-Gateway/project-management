...
class FakeTokenAuthenticator(private val decision: AuthenticationDecision) : TokenAuthenticator {
...
class AuthenticationInterceptorTest {
    ...
    @Test
    fun `missing token returns Error`() = runTest {
    ...
    fun `deny decision returns Error`() = runTest {
        val interceptor = AuthenticationInterceptor(FakeTokenAuthenticator(AuthenticationDecision.Deny("Invalid token")))
    ...
    @Test
    fun `allow decision proceeds chain and sets context attributes`() = runTest {
        val validationResult = AuthValidationResult.Opaque(IntrospectionResult(active = true))
        val interceptor = AuthenticationInterceptor(FakeTokenAuthenticator(AuthenticationDecision.Allow(validationResult)))
        val context = fakeContext()
