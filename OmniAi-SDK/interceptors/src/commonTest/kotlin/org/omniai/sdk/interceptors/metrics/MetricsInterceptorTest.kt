...
class MetricsInterceptorTest {
    @Test
    fun `default histogram and typed attributes are emitted for unary responses`() =
        runTest {
    ...
    @Test
    fun `stream failures still record default latency metric with error tags`() =
        runTest {
            val metricsPort = RecordingMetricsPort()
            val interceptor =
                MetricsInterceptor(
    ...
    @Test
    fun `custom metric instruments are created once and reused across requests`() =
        runTest {
            val metricsPort = RecordingMetricsPort()
            val interceptor =
                MetricsInterceptor(
    ...
    @Test
    fun `default latency metric can be disabled`() =
        runTest {
            val metricsPort = RecordingMetricsPort()
            val interceptor =
                MetricsInterceptor(
    ...
    @Test
    fun `default additional attributes cannot override protected default keys`() =
        runTest {
            val metricsPort = RecordingMetricsPort()
            val interceptor =
                MetricsInterceptor(
...
private fun baseContext(): GatewayContext =
    GatewayContext(
        request =
            CommonRequest(
                provider = Provider.OPENAI,
...
private fun unaryResponse(): CommonResponse =
    CommonResponse(
        provider = Provider.OPENAI,
        id = "resp-1",
        model = "gpt-4o-mini",
