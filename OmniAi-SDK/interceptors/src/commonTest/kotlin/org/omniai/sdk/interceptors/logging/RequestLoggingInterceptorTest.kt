...
class RequestLoggingInterceptorTest {
    ...
    @Test
    fun `unary request logs info with provider and model`() =
        runTest {
            ...
            assertTrue(
                logger.infos.any { it.contains("openai") && it.contains("gpt-4o") },
                "Request info log should contain provider and model. Got: ${logger.infos}",
            )
        }

    @Test
    fun `unary response logs info with response provider and model`() =
        runTest {
            val logger = CapturingLogger()
            val interceptor = RequestLoggingInterceptor(logger)
            ...
            assertTrue(
                logger.infos[1].contains("anthropic") && logger.infos[1].contains("claude-3"),
                "Response log should contain response provider and model. Got: ${logger.infos[1]}",
            )
        }

    @Test
    fun `unary result is passed through unchanged`() =
        runTest {
    ...
    @Test
    fun `error result logs a warning with the error message`() =
        runTest {
            val logger = CapturingLogger()
            val interceptor = RequestLoggingInterceptor(logger)
            ...
            assertTrue(
                logger.warns.any { it.contains("something went wrong") },
                "Warn log should contain error message. Got: ${logger.warns}",
            )
        }

    @Test
    fun `error result is passed through unchanged`() =
        runTest {
    ...
    @Test
    fun `stream completion logs info`() =
        runTest {
            val logger = CapturingLogger()
            val interceptor = RequestLoggingInterceptor(logger)
            ...
            assertTrue(
                logger.infos.any { it.contains("stream") },
                "Completed stream should log an info. Got: ${logger.infos}",
            )
        }

    @Test
    fun `stream failure logs error`() =
        runTest {
            val logger = CapturingLogger()
            val interceptor = RequestLoggingInterceptor(logger)
            ...
            assertTrue(
                logger.errors.any { it.contains("stream exploded") },
    ...
    @Test
    fun `NoResult is passed through without any logs beyond the initial request log`() =
        runTest {
            val logger = CapturingLogger()
            val interceptor = RequestLoggingInterceptor(logger)
    ...
    @Test
    fun `default NoOp logger does not throw`() =
        runTest {
