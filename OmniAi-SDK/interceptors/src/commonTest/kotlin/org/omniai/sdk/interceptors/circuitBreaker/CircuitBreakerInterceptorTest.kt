...
class CircuitBreakerInterceptorTest {
    ...
    private fun outbound(provider: Provider = Provider.OPENAI, model: String = "gpt-4o") =
    ...
    @Test
    fun `CLOSED circuit passes request to next chain`() = runTest {
    ...
    @Test
    fun `CLOSED circuit records success and stays CLOSED on success`() = runTest {
    ...
    @Test
    fun `OPEN circuit immediately returns Error without calling chain`() = runTest {
        val store = InMemoryCircuitBreakerStore()
    ...
    @Test
    fun `OPEN circuit adds outbound to denied set in context attributes`() = runTest {
        val store = InMemoryCircuitBreakerStore()
        store.transitionState("openai:gpt-4o", CircuitState.OPEN)
    ...
    @Test
    fun `failures at threshold transition circuit to OPEN`() = runTest {
        val store = InMemoryCircuitBreakerStore()
    ...
    @Test
    fun `failures below threshold do not open circuit`() = runTest {
        val store = InMemoryCircuitBreakerStore()
    ...
    @Test
    fun `success in HALF_OPEN transitions circuit to CLOSED`() = runTest {
        val store = InMemoryCircuitBreakerStore()
    ...
    @Test
    fun `no matching outbound passes request through without state changes`() = runTest {
        val chain = StaticChain(PipelineResult.Unary(fakeResponse()))
    ...
    @Test
    fun `store starts CLOSED with zero failures`() = runTest {
        val store = InMemoryCircuitBreakerStore()
    ...
    @Test
    fun `transitionState to CLOSED resets failures`() = runTest {
        val store = InMemoryCircuitBreakerStore()
    ...
    @Test
    fun `different outbound IDs are tracked independently`() = runTest {
        val store = InMemoryCircuitBreakerStore()
