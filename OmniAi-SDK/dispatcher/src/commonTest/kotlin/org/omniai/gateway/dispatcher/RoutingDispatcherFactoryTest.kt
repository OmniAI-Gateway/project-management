...
class RoutingDispatcherFactoryTest {
    ...
    @Test
    fun `should route unary request to the matching outbound port`() =
        runTest {
            // Arrange
            val expectedResponse =
                success(CommonResponse(provider = openAiProvider, id = "id1", model = gpt4Model.model, choices = emptyList()))
            val targetOutbound =
                FakeOutboundAdapter(provider = openAiProvider, model = gpt4Model).apply {
            ...
            val request =
                CommonRequest(provider = openAiProvider, model = gpt4Model.model, messages = emptyList(), providerOptions = TypedMap())
    ...
    @Test
    fun `should return UnknownDomainError when no outbound port matches unary request`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `should merge attributes into providerOptions before calling outbound port`() =
        runTest {
            // Arrange
            val targetOutbound =
                FakeOutboundAdapter(provider = openAiProvider, model = gpt4Model).apply {
                    stubbedUnaryResponse =
                        success(CommonResponse(provider = openAiProvider, id = "id1", model = gpt4Model.model, choices = emptyList()))
                }
            ...
            val request =
                CommonRequest(provider = openAiProvider, model = gpt4Model.model, messages = emptyList(), providerOptions = initialOptions)
    ...
    @Test
    fun `should route stream request to the matching outbound port`() =
        runTest {
            // Arrange
            val expectedStream = success(emptyFlow<CommonResponseEvent>())
            val targetOutbound =
                FakeOutboundAdapter(provider = anthropicProvider, model = claudeModel).apply {
                    stubbedStreamResponse = expectedStream
                }

            val dispatcher = routingDispatcherFactory(listOf(targetOutbound))
            val request =
                CommonRequest(provider = anthropicProvider, model = claudeModel.model, messages = emptyList(), providerOptions = TypedMap())
