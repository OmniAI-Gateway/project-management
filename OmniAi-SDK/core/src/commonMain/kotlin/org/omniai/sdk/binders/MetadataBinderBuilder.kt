...
class MetadataBinderBuilder {
    ...
    abstract inner class BaseBindingContext(
        ...
        fun <T : Any> bindTo(domainKey: AttributeKey<T>, decode: (String) -> T?) {
        ...
        private fun <T : Any> addBinding(domainKey: AttributeKey<T>, decode: (String) -> T?) {
            bindings.add(BindingSpec(source, key, domainKey, decode))
        }
    }

    inner class HeaderContext(key: String) : BaseBindingContext(Source.HEADER, key)

    inner class QueryContext(key: String) : BaseBindingContext(Source.QUERY, key)

    inner class PathContext(key: String) : BaseBindingContext(Source.PATH, key)

    inner class PropertyContext(key: String) : BaseBindingContext(Source.PROPERTY, key)
    ...
    fun build(): ConfigurableMetadataBinder {
        return ConfigurableMetadataBinder(bindings.toList())
    }
}

fun buildMetadataBinder(block: MetadataBinderBuilder.() -> Unit): ConfigurableMetadataBinder {
    return MetadataBinderBuilder().apply(block).build()
}