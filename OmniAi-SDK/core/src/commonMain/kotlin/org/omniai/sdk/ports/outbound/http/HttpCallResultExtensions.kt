...
fun HttpCallResult<*>.toDomainError(provider: Provider): DomainError =
    when (this) {
        is HttpCallResult.ApiError -> {
            when (code) {
                401 -> {
                    UnauthorizedError(message = "${provider.value} rejected request with status 401: ${message.orEmpty()}")
                }

                403 -> {
                    ForbiddenError(message = "${provider.value} rejected request with status 403: ${message.orEmpty()}")
                }

                404 -> {
                    NotFoundError(message = "${provider.value} rejected request with status 404: ${message.orEmpty()}")
                }

                408 -> {
                    TimeoutError(message = "${provider.value} request timed out: ${message.orEmpty()}")
                }

                409 -> {
                    ConflictError(message = "${provider.value} request conflict: ${message.orEmpty()}")
                }

                429 -> {
                    TooManyRequestsError(message = "${provider.value} rate limit exceeded: ${message.orEmpty()}")
                }
