...
class RestToolExecutor(
    ...
    suspend fun execute(
        ...
        val resolvedUrl =
            pathArguments.entries.fold(tool.targetUrl) { url, (key, value) ->
                url.replace("{$key}", value?.toString() ?: "")
            }

        val response =
            httpClient.request(resolvedUrl) {
                method =
                    when (tool.method.uppercase()) {
                        "GET" -> HttpMethod.Get
                ...
                if (tool.method.uppercase() in listOf("POST", "PUT", "PATCH") && bodyArguments.isNotEmpty()) {
                    ...
                    // Usa a funcao recursiva para converter os dados
                    val jsonBody =
                        buildJsonObject {
