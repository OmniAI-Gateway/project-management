...
class GatewayPipelineTest {
    ...
    @Test
    fun `executeUnary returns Right with response when dispatcher succeeds`() =
        runTest {
    ...
    @Test
    fun `executeUnary delegates request and attributes to dispatcher`() =
        runTest {
    ...
    @Test
    fun `executeUnary returns Left when dispatcher fails`() =
        runTest {
    ...
    @Test
    fun `executeStream returns Right with flow when dispatcher succeeds`() =
        runTest {
            val dispatcher = FakeSuccessDispatcher()
            val pipeline = buildPipeline(dispatcher)
    ...
    @Test
    fun `executeStream delegates to dispatcher generateStream`() =
        runTest {
            val dispatcher = FakeSuccessDispatcher()
            val pipeline = buildPipeline(dispatcher)
    ...
    @Test
    fun `executeStream returns Left when dispatcher fails`() =
        runTest {
    ...
    @Test
    fun `interceptor can modify context before passing to next`() =
        runTest {
    ...
    @Test
    fun `when context res is Error at end of chain, error is propagated`() =
        runTest {
            val error = fakeError("pre-set error")
            val pipeline = buildPipeline(FakeSuccessDispatcher())

            val result =
                pipeline.executeUnary(
    ...
    @Test
    fun `when context res is Unary at end of chain, dispatcher re-runs and overwrites`() =
        runTest {
    ...
    @Test
    fun `executeUnary returns Left when pipeline produces a Stream result`() =
        runTest {
            val pipeline =
                buildPipeline(FakeSuccessDispatcher()) {
    ...
    @Test
    fun `executeStream returns Left when pipeline produces a Unary result`() =
        runTest {
            val pipeline =
                buildPipeline(FakeSuccessDispatcher()) {
    ...
    @Test
    fun `pipeline with no interceptors calls dispatcher directly`() =
        runTest {
            val dispatcher = FakeSuccessDispatcher()
