...
class GatewayPipelineChainTest {
    ...
    @Test
    fun `chain with no interceptors calls dispatcher generate for UNARY mode`() =
        runTest {
    ...
    @Test
    fun `chain with no interceptors calls dispatcher generateStream for STREAM mode`() =
        runTest {
    ...
    @Test
    fun `chain returns Error when dispatcher generate fails`() =
        runTest {
    ...
    @Test
    fun `chain returns Error when dispatcher generateStream fails`() =
        runTest {
    ...
    @Test
    fun `chain returns Error immediately when context res is Error and no interceptors remain`() =
        runTest {
    ...
    @Test
    fun `chain re-dispatches when context res is Unary and no interceptors remain`() =
        runTest {
    ...
    @Test
    fun `chain re-dispatches when context res is Stream and no interceptors remain`() =
        runTest {
            val dispatcher = FakeSuccessDispatcher()
            val chain = GatewayPipelineChain(emptyList(), dispatcher, 0)
    ...
    @Test
    fun `interceptors are invoked in order and each sees the incremented index`() =
        runTest {
    ...
    @Test
    fun `interceptor at end of list delegates to dispatcher`() =
        runTest {
            val dispatcher = FakeSuccessDispatcher()
            val interceptors =
                listOf(
