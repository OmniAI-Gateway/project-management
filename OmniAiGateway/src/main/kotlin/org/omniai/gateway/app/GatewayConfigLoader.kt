...
fun loadGatewayConfig(): GatewayConfig =
    ConfigFactory.load().let { appConfig ->
        val providers = ProviderKind.entries.mapNotNull { kind ->
        ...
        GatewayConfig(
            ...
            providers = providers,
            telemetryEnabled = appConfig.firstBoolean(
                paths = listOf("gateway.telemetryEnabled", "gateway.metrics.enabled"),
                defaultValue = true
            ),
            otelEnabled = appConfig.firstBoolean(
...
private fun Config.safeBoolean(path: String, defaultValue: Boolean): Boolean =
    runCatching { getBoolean(path) }.getOrDefault(defaultValue)

private fun Config.firstString(paths: List<String>, defaultValue: String): String =
    paths.firstNotNullOfOrNull { path ->
        if (hasPath(path)) runCatching { getString(path) }.getOrNull() else null
    } ?: defaultValue

private fun Config.firstBoolean(paths: List<String>, defaultValue: Boolean): Boolean =
    paths.firstNotNullOfOrNull { path ->
...
private fun Config.loadProviderConfig(kind: ProviderKind): ProviderConfig? {
    ...
    val baseUrl = System.getenv(kind.baseUrlEnv)
        ?: safeString("$pathPrefix.baseUrl", "")
    require(baseUrl.isNotBlank()) {
        "Missing baseUrl for provider '${kind.configKey}' at '$pathPrefix.baseUrl'"
    }

    val modelsFromEnv = System.getenv(kind.modelsEnv)
        ?.split(',')
        ?.map { it.trim() }
        ?.filter { it.isNotEmpty() }
        .orEmpty()

    val modelsFromConfigList = runCatching {
...
private fun Config.readStringValues(path: String): List<String> {
    ...
    return singleValue?.trim()?.takeIf { it.isNotEmpty() }?.let(::listOf).orEmpty()
}
