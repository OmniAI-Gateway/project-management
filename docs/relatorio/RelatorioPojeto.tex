...
% ================= CAPÍTULOS DO RELATÓRIO =================
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            % --- CLIENTES ---
            \node[box] (clients) {Clientes externos\\REST Client, Claude Code, SDKs};

            % --- OMNIAI GATEWAY (Grupo) ---
            \node[inner, right=1.5cm of clients, yshift=1.2cm] (config) {Configuração do POC\\ambiente e \texttt{application.conf}};
            \node[inner, below=0.5cm of config] (ktor) {Conectores Ktor\\rotas HTTP compatíveis};
            \node[sdk, below=0.5cm of ktor] (sdk) {OmniAI SDK\\domínio, contracts, ports, adapters};
