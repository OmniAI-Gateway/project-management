...
% ================= CAPÍTULOS DO RELATÓRIO =================
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.5cm}|Y|Y|Y|}
            ...
            \hline
            Utilização de tokens & Todas expõem algum tipo de contabilização de utilização. & OpenAI usa \texttt{prompt\_tokens}/\texttt{completion\_tokens}; Anthropic usa \texttt{input\_tokens}/\texttt{output\_tokens}; Gemini usa \texttt{usageMetadata}. & \texttt{CommonUsage}, com tokens de entrada, saída e total quando disponíveis. \\
            \hline
        \end{tabularx}
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.4cm}|>{\raggedright\arraybackslash}p{1.8cm}|Y|Y|}
            ...
            \hline
            Declaração de ferramenta & Gemini & \texttt{functionDeclarations[].parameters}. & Necessidade de traduzir declarações Gemini para a mesma estrutura comum. \\
            \hline
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            box/.style={rectangle, draw, rounded corners, align=center, text width=3.2cm, minimum height=1.0cm, inner sep=5pt},
            common/.style={rectangle, draw, rounded corners, align=center, text width=4.0cm, minimum height=1.2cm, inner sep=5pt, fill=gray!10},
            arrow/.style={-Latex, thick}
        ]
            \node[box] (openai) {OpenAI\\ \texttt{messages}\\ \texttt{tool\_calls}\\ \texttt{delta}};
            \node[box, below=of openai] (anthropic) {Anthropic\\ \texttt{messages}\\ \texttt{tool\_use}\\ eventos nomeados};
            \node[box, below=of anthropic] (gemini) {Gemini\\ \texttt{contents/parts}\\ \texttt{functionCall}\\ \texttt{generationConfig}};

            \node[common, right=1.3cm of anthropic] (domain) {Domínio comum\\ \texttt{CommonRequest}\\ \texttt{CommonResponse}\\ \texttt{CommonResponseEvent}};

            \node[box, right=1.3cm of domain] (provider) {Adapter de saída\\ contrato do fornecedor\\ transporte HTTP/SSE};
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            box/.style={rectangle, draw, rounded corners, align=center, text width=3.8cm, minimum height=1.0cm, inner sep=5pt},
            common/.style={rectangle, draw, rounded corners, align=center, text width=3.8cm, minimum height=1.0cm, inner sep=5pt, fill=gray!10},
            arrow/.style={-Latex, thick}
        ]
            \node[box] (inReq) {Pedido recebido\\OpenAI, Anthropic ou Gemini};
            \node[common, right=of inReq] (commonReq) {\texttt{CommonRequest}\\pedido interno};
            ...
            \node[common, left=of providerResp] (commonResp) {\texttt{CommonResponse}\\ou \texttt{CommonResponseEvent}};
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            root/.style={rectangle, draw, rounded corners, align=center, text width=4.2cm, minimum height=0.9cm, fill=gray!10, inner sep=5pt},
            part/.style={rectangle, draw, rounded corners, align=left, text width=4.4cm, minimum height=1.1cm, inner sep=5pt},
            arrow/.style={-Latex, thick}
        ]
            \node[root] (request) {\texttt{CommonRequest}};
            \node[part, below left=of request] (identity) {\textbf{Destino}\\\texttt{provider}\\\texttt{model}};
            \node[part, below=of request] (conversation) {\textbf{Conversação}\\\texttt{systemPrompt}\\\texttt{messages[]} com \texttt{RequestContentPart}};
            \node[part, below right=of request] (tools) {\textbf{Ferramentas e opções}\\\texttt{tools[]}\\\texttt{toolChoice}\\\texttt{providerOptions}};
            \node[part, below=0.8cm of conversation] (generation) {\textbf{Geração}\\\texttt{generationConfig}\\temperatura, limites, \textit{stop} e formato de resposta};

            \draw[arrow] (request) -- (identity);
            \draw[arrow] (request) -- (conversation);
            \draw[arrow] (request) -- (tools);
            \draw[arrow] (request) -- (generation);
        \end{tikzpicture}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            ...
            box/.style={rectangle, draw, rounded corners, align=center, text width=3.3cm, minimum height=1.0cm, inner sep=5pt},
            inner/.style={rectangle, draw, rounded corners, align=center, text width=4.2cm, minimum height=0.9cm, fill=white, inner sep=5pt},
            group/.style={rectangle, draw, rounded corners, inner sep=8pt},
            arrow/.style={-Latex, thick}
        ]
            \node[box] (clients) {Clientes externos\\REST Client\\Claude Code\\SDKs compatíveis};

            \node[inner, right=1.0cm of clients, yshift=1.0cm] (config) {Configuração do POC\\ambiente e \texttt{application.conf}};
            \node[inner, below=of config] (ktor) {Conectores Ktor\\rotas HTTP compatíveis};
            \node[inner, below=of ktor] (sdk) {OmniAI SDK\\domínio, contracts, ports, adapters};
            \node[group, fit=(config)(ktor)(sdk), label={[align=center]above:\textbf{OmniAI Gateway POC}}] (gatewayGroup) {};

            \node[box, right=1.0cm of ktor] (providers) {Fornecedores de inferência\\Gemini\\Anthropic\\OpenAI-compatible/Groq};
            \node[box, below=0.9cm of providers] (observability) {Operação local\\Logto\\OTel Collector\\Prometheus/Grafana};

            \draw[arrow] (clients.east) -- node[above]{HTTP/JSON/SSE} (gatewayGroup.west);
            \draw[arrow] (gatewayGroup.east) -- node[above]{HTTP/JSON/SSE} (providers.west);
    ...
    O Ktor é usado em dois pontos diferentes, que importa distinguir. Primeiro, o SDK fornece uma implementação HTTP baseada em Ktor, através de \texttt{KtorHttpTransportClient}, usada pelos outbounds para executar pedidos HTTP e SSE. Segundo, o SDK fornece o módulo \texttt{gateway-ktor-server}, que disponibiliza conectores Ktor para expor rotas compatíveis com OpenAI, Anthropic e Gemini. O Ktor não é, portanto, o SDK; é uma implementação concreta fornecida pelo SDK e usada nativamente pelo POC OmniAI Gateway.

    A OmniAI Gateway, por sua vez, é uma aplicação Ktor separada, construída como prova de conceito para demonstrar o uso do SDK num sistema real. O projeto usa Gradle com estrutura multi-módulo e \textit{composite builds} \cite{gradle_composite_builds}. Durante o desenvolvimento, o POC inclui o SDK localmente com \texttt{includeBuild("../OmniAi-SDK")}, acelerando o ciclo de validação: uma alteração no SDK pode ser compilada e executada imediatamente pela aplicação demonstradora.
...
metrics {
    ...
    A Gateway não deve atuar como \textit{Authorization Server}: não emite tokens, não gere credenciais dos clientes e não substitui Logto ou Keycloak. O seu papel é o de API protegida. Por isso, valida tokens recebidos, confirma se foram emitidos para a audiência esperada e só depois permite que o pedido avance para autorização, rate limiting e inferência. Esta separação reduz responsabilidade de segurança dentro do SDK e permite integrar diferentes fornecedores de identidade sem alterar o contrato dos clientes.
    ...
    A autorização é separada da autenticação porque responder a ``quem é o chamador?'' não é o mesmo que responder a ``o que este chamador pode fazer?''. A autenticação valida identidade; a autorização decide permissões sobre uma ação e um recurso. Esta separação é importante numa Gateway multi-cliente, onde um token válido pode não ter permissão para usar todos os modelos, fornecedores ou operações. A distinção entre autenticação e autorização é também a base dos modelos modernos de identidade: a primeira prova identidade, enquanto a segunda concede permissão sobre dados ou ações protegidas \cite{microsoft_authn_authz}.

    No contexto do projeto, esta separação evita dois problemas. Primeiro, impede que a validação de token seja confundida com autorização total: um cliente autenticado pode estar limitado a um subconjunto de modelos, fornecedores ou quotas. Segundo, permite trocar a origem das decisões de autorização sem alterar a autenticação. Hoje a decisão pode ser local ou simples; no futuro pode vir de AuthZen, OPA ou outro PDP externo.

    No OmniAI SDK, a autorização é centralizada no \texttt{PolicyEnforcerInterceptor}. O componente atua como \textit{Policy Enforcement Point} (PEP): interceta o pedido, constrói um \texttt{AuthorizationInput} e delega a decisão a um \textit{Policy Decision Point} (PDP) através da porta \texttt{PolicyDecisionPointPort}.
    ...
    No target JVM, o SDK pode ser utilizado por aplicações Kotlin ou Java. Este é o target mais consolidado no estado atual do projeto e é o usado pelo POC OmniAI Gateway. A implementação JVM inclui Ktor HTTP client, integração com servidor Ktor, autenticação com descoberta OIDC, telemetria e execução real de outbounds. A distribuição futura via Maven permitirá que uma aplicação adicione o SDK como dependência versionada, sem depender de \textit{composite build}.
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[
            node distance=0.8cm and 1.0cm,
            box/.style={rectangle, draw, rounded corners, align=center, text width=3.4cm, minimum height=0.9cm, inner sep=5pt},
            gateway/.style={rectangle, draw, rounded corners, align=center, text width=4.0cm, minimum height=2.0cm, inner sep=6pt, fill=gray!10},
            arrow/.style={-Latex, thick}
        ]
            \node[box] (rest) {REST Client\\ \texttt{gateway-requests.http}};
            \node[box, below=of rest] (claude) {Claude Code\\ \texttt{ANTHROPIC\_BASE\_URL}};
            \node[box, below=of claude] (node) {SDK Anthropic\\ Node.js};

            \node[gateway, right=1.4cm of claude] (gateway) {OmniAI Gateway POC\\ Ktor + OmniAI SDK\\ Logto + Telemetria};

            \node[box, right=1.4cm of gateway, yshift=0.8cm] (providers) {Fornecedor configurado\\ Gemini\\OpenAI-like\\Anthropic};
            \node[box, right=1.4cm of gateway, yshift=-0.8cm] (obs) {Prometheus + Grafana\\ via OpenTelemetry Collector};
    ...
    Foram estudados e testados cenários com clientes que permitem alterar o endpoint da API. No caso do Claude Code, a variável \texttt{ANTHROPIC\_BASE\_URL} permite redirecionar pedidos para a Gateway. No caso do SDK oficial da Anthropic em Node.js, foi criado um teste em \texttt{node-tests/anthropicSDKTest.js}, configurado com \texttt{baseURL} local e chave fictícia, para validar que um cliente existente consegue falar com a OmniAI Gateway quando esta expõe um contrato compatível.
