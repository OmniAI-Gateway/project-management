...
% ================= ELEMENTOS PRÉ-TEXTUAIS =================
    \begin{abstract}
        Os Modelos de Linguagem de Grande Escala (\textit{Large Language Models -- LLMs}) \cite{ibm_llm_topics} são modelos treinados com grandes volumes de texto, capazes de compreender linguagem natural e gerar respostas úteis para tarefas. Na prática, a maioria das aplicações consome LLMs através de APIs de inferência disponibilizadas por fornecedores externos, o que coloca desafios de integração e interoperabilidade quando existem múltiplos contratos e formatos em uso.
    ...
    \begin{abstract}
        Large Language Models (LLMs) \cite{ibm_llm_topics} are models trained on massive volumes of text, capable of understanding natural language and generating useful responses for tasks. In practice, most applications consume LLMs through inference APIs provided by external vendors, which introduces integration and interoperability challenges when multiple contracts and formats are in use.

        The absence of a common protocol for communicating with Large Language Models (LLMs) forces applications to integrate different access interfaces and specific implementations to invoke external tools. To address this technological fragmentation, this project proposes the development of the \textbf{OmniAI SDK}, a Software Development Kit built on Kotlin Multiplatform (KMP). Based on the principles of Hexagonal Architecture (\textit{Ports and Adapters}), the SDK isolates the domain logic and supports bidirectional communication compatible with the standards of reference APIs, namely OpenAI, Anthropic, and Gemini. The core of the system implements an extensible and configurable \textit{pipeline}, inspired by the Servlet filter model, which executes pre-built \textit{interceptors} for authorization, authentication, metrics, \textit{rate limiting}, \textit{circuit breaker}, and \textit{fallback}. Additionally, the SDK integrates an MCP Broker, allowing for the dynamic registration of external tools via configuration. Distributed as a multiplatform solution for JVM environments, via Maven, and Node.js/TypeScript, via NPM, the SDK was validated through the development of the \textbf{OmniAI Gateway}, an end-user application that consumes the library to quickly instantiate a Gateway. The evaluation demonstrates that the solution minimizes technological coupling and integration effort, ensuring resilience, scalability, and centralization in AI model management.
    \end{abstract}
...
% ================= CAPÍTULOS DO RELATÓRIO =================
    ...
    \section{Ecossistema do OmniAI SDK}
