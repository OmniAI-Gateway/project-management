...
\end{table}
    ...
    O modelo de resposta comum, sob a designação \texttt{CommonResponse}, é a base que normaliza as respostas devolvidas pelos diferentes fornecedores, garantindo um contrato único e consistente para o cliente. A sua estrutura foi desenhada para representar o resultado final da geração, mantendo as várias opções de resposta -- as escolhas representadas pela classe \texttt{CommonChoice} -- juntamente com o respetivo índice e a razão de término indicada por \texttt{FinishReason}. Cada escolha contém uma mensagem de resposta, definida em \texttt{CommonResponseMessage}, que estipula o papel da entidade na propriedade \texttt{CommonRole} e o seu conteúdo segmentado em partes através de \texttt{ResponseContentPart}, permitindo assim combinar texto, JSON, chamadas a ferramentas ou recusas do modelo. A informação de utilização isolada em \texttt{CommonUsage} quantifica o consumo de \textit{tokens} e facilita a extração de métricas e contabilização. Por fim, a propriedade \texttt{providerOptions} preserva metadados específicos do fornecedor, mantendo o núcleo do domínio estável e extensível.
