...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Massive spacing to absolutely prevent overlapping on the right side
        \node[sdkBox, below=3.5cm of infGateway] (mcpBroker) {\textbf{MCP Broker}\\ \footnotesize{(Transporte HTTP/STDIO,\\Gestão de Ferramentas)}};
        ...
        \node[interceptorBox, above=0.8cm of infGateway, xshift=-1.4cm] (int1) {Interceptor 1};
        \node[interceptorBox, above=0.8cm of infGateway, xshift=1.4cm] (int2) {Interceptor 2};
        \draw[arrow] (int1.south) -- (int1.south |- infGateway.north);
        \draw[arrow] (int2.south) -- (int2.south |- infGateway.north);

        % SDK Group
        \node[sdkGroupStyle, fit=(infGateway)(mcpBroker), label={[align=center, font=\bfseries]above:OmniAI SDK}] (sdkGroup) {};

        % Grouping them in "Aplicação Gateway"
        \node[group, fit=(entryInf)(sdkGroup)(int1)(int2), label={[align=center, font=\bfseries]above:Aplicação: OmniAI Gateway}] (gatewayGroup) {};
        ...
        \node[box, above=1.2cm of providers] (auth) {\textit{Servidor de Autorização}\\ \footnotesize{(OIDC / OAuth2)}};
        \node[box, below=1.2cm of providers] (telemetry) {Observabilidade e Telemetria\\ \footnotesize{(OpenTelemetry)}};
        
        \coordinate (mcpServersPos) at (gatewayGroup.east |- mcpBroker);
        \node[box, right=1.5cm of mcpServersPos] (mcpServers) {Servidores MCP\\ \footnotesize{(Ferramentas Externas)}};

        % Outputs Inference
        \coordinate (forkInf) at ([xshift=0.7cm]providersPos);
        \draw[thick] (infGateway.east) -- (forkInf);
        \draw[arrow] (forkInf) |- (auth.west);
        \draw[arrow] (forkInf) -- (providers.west);
        \draw[arrow] (forkInf) |- (telemetry.west);

        % Outputs MCP
        \draw[arrow] (mcpBroker.east) -- (mcpServers.west);

    \end{tikzpicture}%
    }
    \caption{Arquitetura de Alto Nível: O \textbf{OmniAI SDK} utilizado para instanciar uma \textbf{OmniAI Gateway}, com a \textbf{Inference Gateway} a abstrair integrações externas padronizadas e o \textbf{MCP Broker} a gerir ferramentas.}
    \label{fig:top-level}
\end{figure}
...
No domínio de inferência, a \texttt{TypedMap} aparece em \texttt{providerOptions}, permitindo preservar dados que não pertencem ao subconjunto comum mas não devem ser perdidos durante a tradução. Fora da inferência estrita, a mesma estrutura permite transportar metadados entre componentes sem criar dependências diretas entre eles. Esta característica é fundamental para a evolução futura do \textbf{OmniAI SDK}, porque permite que mecanismos como autorização, observabilidade ou políticas de execução, consumam e injetem atributos adicionais sem forçar alterações constantes nas classes centrais do domínio.
