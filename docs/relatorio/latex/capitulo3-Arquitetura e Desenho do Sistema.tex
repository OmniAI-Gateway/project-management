...
\end{table}
    ...
    O modelo de resposta comum, sob a designação \texttt{CommonResponse}, é a base que normaliza as respostas devolvidas pelos diferentes fornecedores. A sua estrutura foi desenhada para representar o resultado final da geração, mantendo as várias opções de resposta -- as escolhas representadas pela classe \texttt{CommonChoice} -- juntamente com o respetivo índice e a razão de término indicada por \texttt{FinishReason}. Cada escolha contém uma mensagem de resposta, definida em \texttt{CommonResponseMessage}, que estipula o papel da entidade na propriedade \texttt{CommonRole} e o seu conteúdo segmentado em partes através de \texttt{ResponseContentPart}, permitindo assim combinar texto, JSON, chamadas a ferramentas ou recusas do modelo. A informação de utilização isolada em \texttt{CommonUsage} quantifica o consumo de \textit{tokens} e facilita a extração de métricas e contabilização. Por fim, a propriedade \texttt{providerOptions} preserva metadados específicos do fornecedor.
    \begin{figure}[H]
        \centering
...
\vspace{1em}
    ...
    Esta abordagem revela-se significativamente mais segura do que um \texttt{Map<String, Any?>} genérico, pois cada operação de leitura assegura de imediato o tipo esperado do valor, tal como foi especificado na chave. Em simultâneo, é mais flexível do que adicionar dezenas de campos opcionais diretamente às classes de domínio sempre que surge uma nova funcionalidade no sistema.
    
    Para além da sua utilidade no domínio de inferência, o facto de a estrutura \texttt{TypedMap} ter sido concebida de forma totalmente genérica e agnóstica permitiu o seu reaproveitamento extensivo noutras partes críticas do sistema. Por ser independente de qualquer lógica de negócio específica, tornou-se o mecanismo padrão do \textbf{OmniAI SDK} para transporte seguro de metadados, sendo utilizada tanto no \textit{pipeline} de interceção de pedidos como nos resultados de chamadas \textbf{HTTP} e gestão de sessões do utilizador.
...
No domínio de inferência, a estrutura \texttt{TypedMap} é utilizada na propriedade de domínio comum \texttt{providerOptions}, permitindo preservar dados que não pertencem ao subconjunto padronizado, mas que não devem ser perdidos durante o processo de tradução entre formatos. Fora da inferência estrita, a mesma estrutura permite transportar metadados entre componentes sem criar dependências diretas entre eles. Esta característica é fundamental para a evolução futura do \textbf{OmniAI SDK}, porque permite que mecanismos como autorização, observabilidade ou políticas de execução, consumam e injetem atributos adicionais sem forçar alterações constantes nas classes centrais do domínio.
