    ...
    diferentes interfaces de acesso aos modelos, estruturas específicas de pedidos e respostas, modelos distintos de contabilização de tokens, custos variáveis e implementações particulares para invocação de funções externas (ver Figuras \ref{fig:p1}--\ref{fig:p3}). Por exemplo, a API da OpenAI \cite{openai_api} organiza a interação em torno de chat completions e tool calls; a API da Anthropic \cite{anthropic_api} utiliza uma arquitetura baseada em messages e blocos de conteúdo; e a API do Gemini \cite{google_gemini_api} estrutura a geração através de contents, parts, function declarations e configurações de geração próprias.
    ...
    Para tornar a solução configurável e escalável, foi criada uma \textit{pipeline} interna de \textit{interceptors}, inspirada no modelo de filtros e cadeia de filtros dos Servlets \cite{java_servlet_filter}\cite{java_servlet_filterchain}. Esta \textit{pipeline} permite injetar comportamentos específicos tanto antes como após a chamada ao fornecedor de inferência ou ao próximo \textit{interceptor} na \textit{pipeline}, incluindo autenticação, autorização, métricas, \textit{rate limiting}, \textit{circuit breaker}, \textit{fallback} e \textit{logging}.

    Para garantir que a \textbf{OmniAI Gateway} atue como uma peça de infraestrutura completa de inteligência artificial, a arquitetura incorpora um \textit{MCP Broker}. Este módulo permite a exposição  e centralização de múltiplas ferramentas externas através do \textit{Model Context Protocol} (MCP) \cite{mcp_protocol}.   

    \section{Estrutura do Documento}
    Este documento encontra-se organizado de forma progressiva, partindo do problema de integração e avançando até à validação da solução construída. O Capítulo 2 apresenta o estudo prévio: primeiro a análise comparativa das APIs de inferência e depois o estudo do \textit{Model Context Protocol}. O Capítulo 3 descreve a modelação do domínio comum, explicando como os conceitos observados no estudo foram transformados em estruturas como \texttt{CommonRequest}, \texttt{CommonResponse}, \texttt{CommonResponseEvent}, \texttt{CommonTool} e \texttt{TypedMap}. O Capítulo 4 descreve a arquitetura do \textbf{OmniAI SDK} e do \textbf{Proof of concept}, incluindo a visão top-level, a Arquitetura Hexagonal, os módulos, os \textit{contracts}, os \textit{inbounds}, os \textit{outbounds}, o \textit{dispatcher}, a DSL e o cliente HTTP. O Capítulo 5 detalha a implementação técnica e a evolução do sistema, com foco na separação entre \textbf{OmniAI SDK} e \textbf{OmniAI Gateway}, na \textit{pipeline} de \textit{interceptors}, nos mecanismos de segurança, observabilidade e resiliência, no MCP Broker e na distribuição multiplataforma. O Capítulo 6 apresenta as provas de conceito efetivamente realizadas no Proof of concept OmniAI Gateway. O Capítulo 7 sintetiza as conclusões, limitações e linhas de evolução futura. O Capítulo 8 regista o histórico de planeamento e os principais riscos considerados durante o desenvolvimento.
