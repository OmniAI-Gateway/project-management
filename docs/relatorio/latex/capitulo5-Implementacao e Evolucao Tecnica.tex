...
Os \textit{Inbound Translators} são responsáveis por converter os contratos específicos de cada fornecedor de inferência para o modelo de domínio comum do \textbf{OmniAI SDK}, e realizar o processo inverso na geração de respostas.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\ttfamily\raggedright\arraybackslash}p{4cm}|Y|}
        ...
        \hline
        messages[].role &
        Convertido diretamente para os papéis do domínio comum: \texttt{SYSTEM}, \texttt{ASSISTANT}, \texttt{TOOL} e \texttt{USER} (utilizado por defeito quando omisso). \\
        \hline
        messages[].content \& toolCalls &
        O conteúdo origina instâncias de \texttt{TextPart}. As chamadas de ferramentas são convertidas em \texttt{ToolCallPart(toolCallId, functionName)}, com os argumentos encapsulados num mapa sob a chave \texttt{"raw"}. Quando o papel é \texttt{TOOL}, é gerado um \texttt{ToolResultPart}, convertendo o conteúdo textual para primitivas JSON. \\
        \hline
        tools[].function &
        Convertido para \texttt{CommonTool(name, description, parametersSchema)}, extraindo as propriedades definidas no schema da ferramenta. \\
        \hline
        toolChoice &
        ...
        \hline
        stream, frequencyPenalty, presencePenalty, n, seed, user, logitBias, logProbs, topLogProbs &
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\ttfamily\raggedright\arraybackslash}p{4cm}|Y|}
        ...
        \hline
        id \& created &
        Utiliza o \texttt{id} nativo ou gera um identificador no formato \texttt{chatcmpl-UUID}. O instante temporal é recuperado das \texttt{providerOptions} ou gerado pelo \texttt{Clock.System}. \\
        \hline
        choices[].message.content &
        O conteúdo é obtido através de \texttt{firstRenderableContentOrNull()}, privilegiando o primeiro \texttt{TextPart} e recorrendo a \texttt{RefusalPart} ou à representação textual de \texttt{JsonPart} quando necessário. \\
        \hline
        choices[].message
        .toolCalls &
        ...
        Converte \texttt{STOP} para \texttt{stop}, \texttt{LENGTH} para \texttt{length}, \texttt{TOOL\_CALL} para \texttt{tool\_calls} e \texttt{CONTENT\_FILTER} para \texttt{content\_filter}. \\
        \hline
        usage &
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\ttfamily\raggedright\arraybackslash}p{4.5cm}|Y|}
        ...
        \hline
        system & A lista de \texttt{AnthropicContent} transforma os blocos de texto num \texttt{SystemPrompt}, juntando tudo numa única string através de um \texttt{joinToString()}. Se o conteúdo estiver em branco, é simplesmente ignorado. \\
        \hline
        messages[].role & Convertido de forma direta para os enums de domínio: \texttt{SYSTEM}, \texttt{ASSISTANT}, \texttt{TOOL} ou \texttt{USER}. \\
        \hline
        ...
        \hline
        tools & A lista é mapeada para instâncias de \texttt{CommonTool}. O \texttt{inputSchema} original é convertido num \texttt{JsonObject} genérico e guardado no campo \texttt{parametersSchema}. \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{4.5cm}|Y|}
        ...
        \hline
        \texttt{ResponseStarted} & Retorna um evento \texttt{MessageStart} com metadados estáticos, assumindo o papel de \texttt{role="assistant"}. \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\ttfamily\raggedright\arraybackslash}p{4.5cm}|Y|}
        ...
        \hline
        model & O identificador do modelo é extraído a partir do \textit{path} HTTP. Caso não seja fornecido, assume-se a constante \texttt{"NO MODEL"} como \textit{fallback}. \\
        \hline
        systemInstruction & Convertido para um \texttt{SystemPrompt}, agrupando todas as \texttt{parts} de texto num único bloco contínuo, separadas por quebras de linha. \\
        \hline
        contents[].role & Adapta os papéis nativos para o domínio: \texttt{model} e \texttt{assistant} passam a \texttt{ASSISTANT}, \texttt{function} passa a \texttt{TOOL}, enquanto \texttt{user} se mantém inalterado. \\
        \hline
        contents[].parts & O conteúdo é transformado de acordo com o seu tipo: \texttt{text} gera um \texttt{TextPart}, \texttt{functionCall} origina um \texttt{ToolCallPart}, e \texttt{functionResponse} converte-se em \texttt{ToolResultPart}. \\
        \hline
        ...
        \hline
        toolConfig\newline.functionCallingConfig & O modo de execução é adaptado (ex: \texttt{auto} passa a \texttt{Auto}, \texttt{any} a \texttt{Required}), e as chamadas de funções permitidas são convertidas em escolhas específicas. \\
        \hline
        ...
        \hline
        topK, thinkingConfig & Por se tratarem de configurações mais avançadas e específicas do Gemini, são guardadas no \texttt{TypedMap} (\texttt{providerOptions}) do \texttt{CommonRequest}. \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\ttfamily\raggedright\arraybackslash}p{4cm}|Y|}
        ...
        \hline
        choices[].message\newline.content & De forma simétrica, o conteúdo é traduzido consoante a sua natureza: um \texttt{TextPart} gera o texto base de um \texttt{GeminiResponsePart}; um \texttt{ToolCallPart} preenche o nó correspondente à chamada de função; um \texttt{JsonPart} é convertido numa \texttt{String} com o conteúdo JSON; e um \texttt{RefusalPart} é exposto indicando o motivo pelo qual o modelo recusou a geração. \\
        \hline
        choices[].finishReason & Mapeado diretamente para os motivos de paragem nativos do Gemini: \texttt{STOP}, \texttt{MAX\_TOKENS} ou \texttt{SAFETY}. \\
        \hline
        usage & Os contadores genéricos do domínio são convertidos e agrupados na estrutura \texttt{GeminiUsageMetadata}, alimentando os campos \texttt{promptTokenCount} e \texttt{candidatesTokenCount}. \\
        \hline
    \end{tabularx}
    \caption{Mapeamento Inbound: Domínio para Gemini Generate Content Response Unária}
\end{table}

    Durante o fluxo de \textit{streaming} via Server-Sent Events (SSE), a API do Gemini diferencia-se de forncedores de inferencia como a OpenAI ou Anthropic por não adotar objetos específicos para deltas parciais. Em vez disso, a Google opta por reaproveitar a mesma estrutura da resposta unária (\texttt{Gemini\allowbreak{}Generate\allowbreak{}Content\allowbreak{}Response}) para encapsular cada evento da \textit{stream}. Como consequência, o tradutor é obrigado a mapear os deltas granulares de domínio (\texttt{CommonResponseEvent}) reconstruindo toda a hierarquia do DTO nativo a cada evento emitido, o que exige instanciar \texttt{candidates}, \texttt{content} e \texttt{parts} mesmo quando o objetivo é transmitir apenas um pequeno pedaço de texto.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{4cm}|Y|}
        ...
        \hline
        \texttt{ChoiceFinished} & Emite apenas o motivo de paragem (\texttt{finishReason}) correspondente ao término desta resposta parcial (\textit{choice}). \\
        \hline
        \texttt{UsageReported} & Emite a contagem total de tokens consumidos, encapsulada num objeto \texttt{GeminiUsageMetadata}. \\
        \hline
        ...
        \hline
        \texttt{ResponseErrored} & Caso a geração seja bloqueada pela plataforma, inclui imediatamente um \texttt{GeminiPromptFeedback} a indicar o bloqueio. Erros temporários que permitam novas tentativas (\textit{retries}) são assinalados explicitamente com \texttt{TRANSIENT\_ERROR}. \\
        \hline
    \end{tabularx}
...
\end{table}
    ...
    Os \textit{Outbound Translators} operam no sentido inverso aos \textit{Inbound}, são responsáveis por converter o modelo de domínio comum do OmniAI SDK para os contratos específicos de cada fornecedor de inferência, e realizar o processo inverso na receção de respostas. Além da tradução de pedidos e respostas completas, estes componentes também adaptam eventos incrementais de \textit{streaming} de volta para o formato esperado pelo domínio.
...
No contexto iterativo da reposta não fragmentada da Anthropic (\texttt{Anthropic\allowbreak{}Message\allowbreak{}Response}):
