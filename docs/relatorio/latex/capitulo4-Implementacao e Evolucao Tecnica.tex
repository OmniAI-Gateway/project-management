    ...
    A primeira iteração do projeto consistiu numa \textbf{Gateway} \textbf{HTTP} construída diretamente, responsável por receber pedidos de clientes e encaminhá-los para fornecedores de inferência. Esta fase foi útil para validar rapidamente a viabilidade da solução, testar pedidos reais e compreender diferenças entre as diferentes API's dos fornecedores, neste caso da \textbf{OpenAI} \cite{openai_api}, \textbf{Anthropic}\cite{anthropic_api} e \textbf{Gemini}\cite{google_gemini_api}.
...
Por sua vez, a aplicação \textbf{OmniAI Gateway} tem como principal função carregar as configurações do sistema, inicializar os componentes, associar os conectores do \textbf{Ktor} e expor os \textit{endpoints} da rede. A aplicação serve então como prova de conceito para comprovar que a nossa arquitetura suporta soluções funcionais e totalmente independentes dos fornecedores de IA.

Esta separação teve um impacto direto na preparação do sistema para escalamento horizontal. O \textbf{OmniAI SDK} foi concebido para não guardar dados persistentes locais, mantendo a informação dos pedidos exclusivamente em estruturas transitórias (como o \texttt{GatewayContext} e o \texttt{TypedMap}). Qualquer mecanismo que exija estado partilhado como sistemas de \textit{cache}, controlo de acessos (\textit{rate limiting}), \textit{circuit breakers} ou métricas é delegado através de portas de integração definidas.

Com esta arquitetura, o arranque inicial do projeto pode ser feito de forma muito simples, com os mecanismos auxiliares a correrem diretamente em memória. Contudo, perante a necessidade de distribuir o processamento por vários servidores,a arquitetura permite substituir estas implementações locais por serviços externos partilhados(como o \textit{Redis}\cite{ref_redis} para a \textit{cache}), sem alterar uma única linha das portas.
...
A análise das alternativas partiu de três critérios principais, preservar a estabilidade da versão demonstrável, manter o \textbf{OmniAI SDK} reutilizável e extensível independentemente da aplicação final e evitar que funcionalidades experimentais introduzissem acoplamento excessivo no domínio comum.
...
Esta alteração teve impacto direto na modularidade da arquitetura. Os \textit{inbounds} passaram a depender apenas da porta \texttt{DispatcherPort}; os \textit{outbounds} ficaram responsáveis exclusivamente pela tradução de contratos e comunicação externa; e as preocupações  foram deslocadas para a \textit{pipeline} de \textit{interceptors}. Como consequência, a \textbf{OmniAiGateway} deixou de representar uma implementação acoplada à infraestrutura interna e passou a funcionar como uma composição configurável construída sobre o \textbf{OmniAI SDK}.

    \section{Implementação da Camada HTTP}
    \label{sec:gateway-ktor-server}

    A camada de exposição \textbf{HTTP} de entrada da \textbf{Gateway} (responsável por receber os pedidos dos clientes) é implementada no módulo \texttt{gateway-ktor-server}. Este módulo estabelece a ponte entre o servidor web (\textbf{Ktor}) e os \textit{inbound adapters} do domínio, garantindo que o core do sistema permanece agnóstico em relação à tecnologia de transporte de entrada.
...
\end{figure}
    ...
    A \textit{pipeline} é composta por \texttt{GatewayPipeline}, \texttt{Gateway\-Pipeline\-Chain}, \texttt{Interceptor} e \texttt{Gateway\allowbreak Context}. O \texttt{GatewayContext} transporta o pedido comum, atributos tipados e modo de execução, podendo representar chamadas síncronas ou \textit{streaming}. A cadeia executa os \textit{interceptors} por ordem e, no fim, invoca o \textit{dispatcher}\allowbreak~terminal.

    Esta abordagem também prepara o escalamento horizontal. A \textit{pipeline} não exige estado global obrigatório; cada pedido transporta o seu contexto. Onde existe estado partilhável, como \textit{rate limiting} ou \textit{circuit breaker}, o \textbf{SDK} define \textit{stores} substituíveis. 
    ...
    Esta estrutura permite executar operações antes e depois da inferência, sem acoplar essa lógica aos \textit{adapters} de entrada, aos \textit{adapters} de saída ou ao \textit{dispatcher}.
    ...
    A arquitetura segue inversão de dependência através da porta \texttt{MetricsPort}. O \textbf{OmniAI SDK} define primitivas de métricas comuns, como \texttt{counter}, \texttt{histogram} e \texttt{upDownCounter}, enquanto o \textit{adapter} concreto no \textit{target} \textbf{JVM} exporta para \textbf{OpenTelemetry} \cite{opentelemetry_otlp}. O uso do \textbf{OpenTelemetry Protocol} (\textbf{OTLP}) foi escolhido porque o \textbf{OTLP} define a codificação, transporte e entrega de dados de telemetria entre fontes, coletores e \textit{backends}. Esta escolha evita o acoplamento do \textbf{OmniAI SDK} a ferramentas específicas como \textbf{Prometheus} \cite{prometheus_ref}, \textbf{Grafana} \cite{grafana_ref}, \textbf{Datadog} \cite{ref_datadog} ou outro \textit{backend} específico. O \textbf{SDK} emite telemetria num formato aberto, o Collector recebe e processa esses dados, e a infraestrutura decide para onde os exportar.
...
metrics {
    ...
    \subsection{Interceptor de \textit{Tracing}}

    O \texttt{TracingInterceptor} cria um \textit{span} para cada pedido processado pela \textbf{Gateway}. Quando utilizado em conjunto com o \textbf{OpenTelemetry}, permite acompanhar o percurso de execução do pedido, correlacionando informação como latência, erros e chamadas a serviços externos. Esta capacidade facilita a observação do comportamento da aplicação e a identificação da origem de eventuais falhas ou degradações de desempenho.

    A funcionalidade é abstraída pela interface \texttt{Tracer}, que define o método genérico \texttt{withSpan} para executar um bloco de código no contexto de um \textit{span}. Na plataforma \textbf{JVM}, esta interface é implementada por \texttt{JvmTracer}, responsável pela integração com o \textbf{OpenTelemetry} e pela propagação automática do contexto de \textit{tracing} entre corrotinas. O \textit{TracingInterceptor} é ativado através da configuração do \texttt{tracer}, passando a instrumentar automaticamente os pedidos processados pela \textbf{Gateway}.

    \vspace{0.5em}
    \begin{lstlisting}[language=kotlin, caption={Instalação do \textit{interceptor} de \textit{tracing} via configuração de métricas}]
metrics {
    tracer = telemetryRuntime.tracer
}
    \end{lstlisting}
...
\end{lstlisting}
    ...
    A importância deste \textit{interceptor} é maior do que numa API tradicional, porque cada pedido pode representar custo financeiro direto, consumo de quotas externas e ocupação prolongada de ligações em \textit{streaming}. Sem \textit{rate limiting}, um cliente mal configurado, uma automação em ciclo ou um utilizador abusivo pode sobrecarregar a \textbf{Gateway}, esgotar quotas nos fornecedores e degradar o serviço para clientes legítimos.O \textit{interceptor} permite controlar o consumo de recursos através da definição de limites específicos para cada cliente, plano, modelo ou fornecedor.
    ...
    A implementação atual da \textit{store} em memória mantém um conjunto de \textit{buckets} associados aos limites configurados. Quando um pedido chega, o \textit{interceptor} tenta consumir uma unidade em todos os \textit{buckets} aplicáveis. Se todos permitirem o consumo, o pedido prossegue; caso contrário, é devolvido um \texttt{TooManyRequestsError} com o campo \texttt{retryAfter}, indicando quando o cliente poderá voltar a tentar. Esta informação permite que clientes bem implementados ajustem o ritmo dos pedidos em vez de repetirem tentativas imediatamente. A implementação em memória é suficiente para desenvolvimento ou para uma única instância da \textbf{Gateway}. Em cenários de escalabilidade horizontal, a interface \texttt{RateLimitStore} permite substituir esta implementação por um mecanismo partilhado, como \textbf{Redis}, garantindo que todas as instâncias partilham a mesma informação sobre as quotas disponíveis.
...
interceptors {
    ...
    Quando a chamada prossegue, o \textit{interceptor} observa o resultado. Erros incrementam o contador de falhas; ao atingir o \texttt{failureThreshold}, o circuito passa para o estado \texttt{OPEN}. Após decorrer um período de espera predefinido, o circuito transita do estado \texttt{OPEN} para \texttt{HALF\_OPEN}, permitindo testar um número limitado de pedidos. Se estes forem bem-sucedidos, o circuito regressa ao estado \texttt{CLOSED}; caso ocorram novas falhas, volta ao estado \texttt{OPEN}, evitando inundar um fornecedor que ainda se encontra em recuperação ou que está a aplicar algum tipo de \textit{rate limiting} \cite{microsoft_circuit_breaker}.
...
interceptors {
    ...
    A implementação atual usa a lista de \textit{outbounds} disponíveis. Primeiro tenta o \textit{outbound} correspondente ao fornecedor e modelo que lhe foram passados no contexto; se falhar, acrescenta esse \textit{outbound} ao conjunto de negados e tenta alternativas. Para cada tentativa, cria um novo \texttt{GatewayContext} com os mesmos atributos, mas com \texttt{provider} e \texttt{model} atualizados para o \textit{outbound} alternativo.

    A integração com o \textit{circuit breaker} é feita através do conjunto partilhado de \textit{outbounds} negados. Assim, se um circuito estiver aberto, o \textit{fallback} evita esse destino e tenta outro modelo ou fornecedor. Este desenho permite construir políticas de resiliência, tentando diversos fornecedores e modelos, mantendo a lógica de tentativa num componente só. Ao existir um fallback as métricas e \textit{tracing} registam que ocorreu.
    ...
    A sua ativação é extremamente simples, não requerendo configuração:
...
interceptors {
    ...
    \subsection{Interceptor de \textit{Logging}}
    ...
    \vspace{0.5em}
    \begin{lstlisting}[language=kotlin, caption={Instalação do \textit{interceptor} de \textit{logging}}]
interceptors {
   ...
   \section{Distribuição Multiplataforma}
    ...
    O \textbf{OmniAI SDK} foi desenvolvido com recurso a \textbf{Kotlin Multiplatform (KMP)} \cite{kmp_ref}. A escolha por esta tecnologia deve-se ao objetivo de partilhar a lógica central do sistema por diferentes plataformas sem haver duplicação de código.

    Para concretizar esta ideia, a arquitetura do SDK foi estruturada em duas camadas complementares:
    \begin{itemize}
        \item \textbf{Módulo Comum:} Contem o código que é escrito uma única vez e partilhado por todos os ambientes de execução, garantindo a consistência das regras.
        
        \item \textbf{Módulos Específicos:} Lidam com as funcionalidades de infraestrutura que variam consoante o ecossistema.
    \end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2.5cm,
        % Estilos
        sdkbox/.style={rectangle, rounded corners=8pt, draw=black, thick, minimum width=3.5cm, minimum height=2cm, align=center, font=\Large\bfseries},
        kmpbox/.style={rectangle, draw=black, thick, minimum width=2.5cm, minimum height=2cm, align=center, font=\large\bfseries, inner sep=10pt},
        targetbox/.style={rectangle, draw=black, thick, align=center, inner sep=0pt, minimum width=4.5cm},
        flowarrow/.style={-{Stealth[length=3mm, width=3mm]}, line width=1.5pt, draw=black},
        lbl/.style={fill=white, font=\bfseries, inner sep=2pt, text=black}
    ]

    % Nodos
    \node[sdkbox] (sdk) {OmniAI-SDK};
    
    \node[kmpbox, right=2cm of sdk] (kmp) {Kotlin\\Multiplatform};
    
    \node[targetbox, right=3cm of kmp, yshift=2cm] (jvm) {
        \begin{tabular}{c}
            \rule{0pt}{14pt} \makebox[5.5cm][c]{\large \textbf{TARGET: JVM}} \\[4pt]
            \hline
            \rule{0pt}{18pt} \Large \textbf{Java / Kotlin} \\[6pt]
        \end{tabular}
    };

    \node[targetbox, right=3cm of kmp, yshift=-2cm] (node) {
        \begin{tabular}{c}
            \rule{0pt}{14pt} \makebox[5.5cm][c]{\large \textbf{TARGET: NODE.JS}} \\[4pt]
            \hline
            \rule{0pt}{18pt} \Large \textbf{Node.js} \\[6pt]
        \end{tabular}
    };

    % Setas
    \draw[flowarrow] (sdk) -- (kmp);
    \draw[flowarrow] (kmp.east) -- node[lbl, sloped] {Maven} (jvm.west);
    \draw[flowarrow] (kmp.east) -- node[lbl, sloped] {NPM} (node.west);

    \end{tikzpicture}%
    }
    \caption{Fluxo de distribuição do OmniAI SDK através de Kotlin Multiplatform para os ambientes JVM e Node.js.}
    \label{fig:distribuicao-kmp}
\end{figure}


    Como apresenta a Figura \ref{fig:distribuicao-kmp}, esta separação permite que, a partir de uma base de código unificada, o SDK seja compilado e distribuído para ecossistemas totalmente distintos. 
    No contexto da \textbf{JVM}, o SDK pode ser integrado diretamente em aplicações \textbf{Java} ou \textbf{Kotlin}, sendo esta a implementação utilizada na \textbf{OmniAI Gateway}.
    Para o ambiente JavaScript/Node.js, o SDK pode ser utilizado em aplicações JavaScript e TypeScript. Nestes casos, é utilizada a anotação \texttt{@JsExport} do Kotlin, para expor o código de forma simples para JS.

    Assim, a utilização de KMP permitiu construir um único SDK que pode ser consumido por diferentes \textit{targets} (\textbf{JVM} e \textbf{Node.js}), simplificando a reutilização de código e a sua integração.
