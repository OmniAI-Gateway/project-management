...
Um dos objetivos do \textbf{OmniAI SDK} é permitir que a lógica central da abstração de modelos seja reutilizada em diferentes ambientes de execução. Ao desemvolvermos o \textbf{OmniAI SDK} em \textbf{Kotlin Multiplatform}\cite{kmp_ref} permite separar o código agnóstico das implementações específicas: o domínio, os contratos, as portas, os adaptadores principais, a DSL e parte dos \textit{interceptors} residem em \textit{source sets} comuns; por sua vez, os detalhes de infraestrutura como o cliente \textbf{HTTP}, a reflexão de prioridades, a integração com o servidor \textbf{Ktor} e as pontes de plataforma ficam isolados em \textit{source sets} específicos.
