...
Anthropic com o objetivo de normalizar a forma como aplicações consumidoras de IA disponibilizam
...
inicialização da sessão. O cliente inicia este processo enviando uma mensagem
\textbf{JSON-RPC}, na qual declara a versão do protocolo
suportada e as suas próprias capacidades (por exemplo, suporte para
\textit{sampling} iniciado pelo servidor). O servidor responde indicando a
...
\caption{Fluxo de comunicação MCP: inicialização por \textit{handshake}
JSON-RPC, seguida da descoberta e invocação de cada tipo de primitivo
(\textit{tools}, \textit{resources} e \textit{prompts}). Estes três blocos
partilham a mesma sessão e podem ser exercidos pelo cliente em qualquer
ordem, consoante as capacidades anunciadas pelo servidor na fase de
inicialização.}
