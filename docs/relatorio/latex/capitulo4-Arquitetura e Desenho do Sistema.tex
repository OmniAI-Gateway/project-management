...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % --- OMNIAI GATEWAY (Instância) ---
        \node[inner, right=1.5cm of clients, yshift=1.5cm] (config) {Instância da Gateway (PoC)};
        \node[inner, below=0.6cm of config] (ktor) {Gateway Endpoints HTTP\\(Ktor)};
        \node[sdk, below=0.6cm of ktor] (sdk) {\textbf{OmniAI SDK}\\ \footnotesize{(Core, DSL, Pipeline, Adapters)}};

        % Setas internas lógicas (A instância usa os endpoints que expõem o SDK)
        ...
        \node[group, fit=(config)(ktor)(sdk), label={[align=center]above:\textbf{Aplicação: OmniAI Gateway}}] (gatewayGroup) {};

        % --- LADO DIREITO ---
        \node[box, right=1.5cm of ktor] (providers) {Fornecedores de inferência\\Gemini, Anthropic, OpenAI/Groq};
        \node[box, below=0.8cm of providers] (observability) {Operação e Telemetria\\Logto, OTel, Prometheus};
        ...
        % Saídas do grupo
        \draw[arrow] (gatewayGroup.east) |- (providers.west);
        \draw[arrow] (gatewayGroup.east) |- (observability.west);

    \end{tikzpicture}
    \caption{Arquitetura \textit{top-level}: O OmniAI SDK utilizado para instanciar uma OmniAI Gateway (configurada como PoC).}
    \label{fig:top-level}
\end{figure}
...
Neste contexto, a Arquitetura Hexagonal revelou-se a solução mais adequada para os objetivos do projeto. A abordagem oferece modularidade, isolamento e flexibilidade suficientes para lidar com as diferentes políticas transversais exigidas por uma \textit{Gateway} de Inteligência Artificial, como autenticação, resiliência, observabilidade e roteamento de pedidos, mantendo ao mesmo tempo uma estrutura organizada e simples de evoluir \cite{api_gateway_design, ai_gateway_pattern, karl_ai_gateway}. Além disso, esta organização arquitetural facilita a introdução de novos fornecedores e novas funcionalidades sem comprometer a estabilidade do núcleo da aplicação.
    ...
    A implementação concreta fornecida pelo \textit{SDK}, \texttt{KtorHttpTransportClient}, assenta no cliente HTTP Ktor e reside no módulo \texttt{contracts:ktor-http}. Esta implementação suporta retentativas configuráveis com atraso entre tentativas, tratamento explícito de exceções de rede e de serialização, e integração com SSE nativo do Ktor. A configuração do cliente HTTP subjacente é realizada através de \texttt{PlatformHttpClient}, que inclui \textit{timeouts} de ligação e de \textit{socket}, suporte a \textit{Content Negotiation} e o \textit{plugin} SSE. Esta configuração é fornecida por implementações específicas de plataformaneste caso JVM e JavaScript, permitindo ao \textit{SDK} operar como uma biblioteca \textit{Kotlin Multiplatform}.

    A separação entre a interface \texttt{HttpTransportClient} e a sua implementação em Ktor tem implicações práticas relevantes. Em contextos de teste, os \textit{outbounds} podem ser exercitados com um cliente fictício sem realizar chamadas reais. Em contextos de produção, é possível substituir a implementação padrão por uma versão instrumentada com métricas, por um cliente com configurações específicas de \textit{proxy} ou por uma implementação adaptada a um ambiente de execução distinto, sem alterar qualquer lógica de adaptação ou de domínio.
